    ...
    <div class="modal-dialog  modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header" >
          <h1 class="modal-title text-center w-100 fs-5" id="exampleModalLabel">  Desativar  {{$linha->nomeLinha}} ?</h1>
        ...
        <div class="modal-body text-center gap-3 align-items-center justify-content-center d-flex mt-2">
            <form action="{{route('linhas.status.update', $linha->id)}}" method="post">
                @csrf
                @method('PUT')
            <input type="hidden" name="statusLinha" value="Inativa">
            <button type="submit" class="btn btn-danger">Sim</button>
            <button type="button"  data-bs-dismiss="modal" aria-label="Close" class="btn btn-primary">Não</button>
            </form>
        </div>
        <div class="modal-footer">
          <small class="text-center">Atenção ao desativar essa linha, todos os carros e catracas também serão destivados.
            A linha poderá ser ativada novamente 
          </small>
        </div>
      </div>
    </div>
